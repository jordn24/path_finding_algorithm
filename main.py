...
blueprint = np.pad(np.zeros((height-2, width-2)), (1, 1), 'constant',
                   constant_values=(wall, wall))  # a map provided to the bot network
...
# a map the bot network uses to keep track of where bots have visited
explore_map = blueprint

# initialize positions, dest, route, and exploration map
for i in range(num_bots):
    bots[i]['current_position'] = [math.floor(
        height/2), math.floor(width/2)]  # current position of bot
    explore_map[bots[i]['current_position'][0],
                bots[i]['current_position'][1]] = mapped
...
def get_unexplored_areas(explore_map, unmapped_value):
    ...
    for x in range(0, len(explore_map)):
        for y in range(0, len(explore_map)):
            if explore_map[x][y] == unmapped_value:
                values = [x, y]
                unexplored_areas.append(values)
...
def get_new_destination(current_position, unexplored_areas):
    '''
    Picks closest unexplored area as the new destination dest.
    e.g. sqrt((x1-x2)^2 + (y1-y2)^2).
    '''

    closestPos = [unexplored_areas[0][0], unexplored_areas[0][1]]
    curr_distance = sqrt((unexplored_areas[0][0]-current_position[0])
                         ** 2 + (unexplored_areas[0][1]-current_position[1]) ** 2).real

    for i in range(1, len(unexplored_areas)):
        new_distance = sqrt((unexplored_areas[i][0]-current_position[0])
                            ** 2 + (unexplored_areas[i][1]-current_position[1]) ** 2).real
        if(new_distance < curr_distance):
            curr_distance = new_distance
            closestPos = unexplored_areas[i]
...
def update_explore_map(dest, route, explore_map, planned, unmapped):
    '''
    Locations specified in route and dest are marked as PLANNED only if it was previous UNMAPPED in the explore_map
    variable.
    '''
    for i in range(0, len(route)):
        if explore_map[route[i][0]][route[i][1]] == unmapped:
            explore_map[route[i][0]][route[i][1]] = planned
...
for itr in range(max_itr):
    ...
    for botNum in range(num_bots):
        ...
        if len(dest) == 0:
            ...
            # calculate bot's new destination
            ...
            # mark the location in explored_map as planned if it was unmapped
            explore_map = update_explore_map(
                dest, route, explore_map, planned, unmapped)

        # Using the calculated route, move the bot 1 step towards the destination bot's destination
        curPos, route, dest, explore_map = update_position(
            curPos, route, dest, explore_map, mapped)
    ...
    for i in range(num_bots):
        fig = plt.gcf()
        ax = fig.gca()
        ax.add_patch(patches.Rectangle((bots[i]['current_position'][1]-.5, bots[i]
                                        ['current_position'][0]-.5), 1, 1, edgecolor='blue', facecolor='red', fill=True))
        if len(bots[i]['destination']) != 0:
            plt.plot([b[1] for b in bots[i]['route']], [b[0]
                                                        for b in bots[i]['route']], color='red')
            plt.plot(bots[i]['destination'][1], bots[i]
                     ['destination'][0], color='yellow')
